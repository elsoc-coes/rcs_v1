...
elsoc_long_2016_2022 %>% 
  ...
  ggtitle('Preferencias electorales en plebiscito de entrada y salida') +
  labs(caption = 'La categoría otros incorpora personas que declararon no votar, NS/NR, nulos y blancos.')
...
elsoc_long_2016_2022 %>% 
  filter(tipo_atricion %in% c(1, 33), ola %in% 5:6, , !is.na(pp_3)) %>% 
  mutate(salida = factor(car::recode(salida, "1 = 1; 2 = 2; 3:5 = 3"))) %>%
  mutate(c44 = case_when(c44 == 1 ~ 1,
                            c44 == 2 ~ 2,
                            c44 == 3 ~ 3,
...
elsoc_long_2016_2022 %>%
  ...
  ggplot(aes(y = prop, x = ola, color = pp_3, group = pp_3,
             label = scales::percent(prop, accuracy = .1))) +
  theme_bw() + 
  geom_text_repel(nudge_y = .02, size = 3) +
  geom_point(size = 1.75) + 
  geom_line() + 
...
elsoc_long_2016_2022 %>%
  ...
  mutate(pp_3 = factor(pp_3,
                       labels = c("Votante\nHabitual", "No-votante", "Indefinido"))) %>%
  prop(c15 == 12, by = c(ola, pp_3), na.rm = T)  %>%
  ggplot(aes(y = prop, x = ola, color = pp_3, group = pp_3,
             label = scales::percent(prop, accuracy = .1))) +
  theme_bw() + 
  geom_text_repel(nudge_y = .025, size = 3) +
...
elsoc_long_2016_2022 %>%
  filter(tipo_atricion == 1 | tipo_atricion == 33) %>% 
  as_label(ola) %>%
  filter(!is.na(c12_03), !is.na(pp_3)) %>%
  mutate(pp_3 = factor(pp_3,
                       labels = c("Votante\nHabitual", "No-votante", "Indefinido"))) %>%
  prop(c12_03 %in% 2:3, by = c(ola, pp_3), na.rm = T)  %>%
  ggplot(aes(y = prop, x = ola, color = pp_3, group = pp_3,
             label = scales::percent(prop, accuracy = .1))) +
  theme_bw() + 
...
elsoc_long_2016_2022 %>% 
  filter(tipo_atricion == 1 | tipo_atricion == 33) %>%
  as_label(ola) %>%
  filter(!is.na(c05_03), !is.na(c05_05), !is.na(pp_3)) %>%
  dplyr::select(c05_03, c05_05, ola, ponderador02, segmento_disenno, estrato_disenno, pp_3) %>% 
...
elsoc_long_2016_2022 %>% 
  filter(tipo_atricion == 1 | tipo_atricion == 33) %>%
  as_label(ola) %>%
  filter(!is.na(c37_03), !is.na(c37_04), !is.na(c37_05), !is.na(pp_3)) %>%
  dplyr::select(c37_03, c37_04, c37_05,  ola, ponderador02, segmento_disenno, estrato_disenno, pp_3) %>% 
