...
tab <- elsoc_long_2016_2022 %>% 
  ...
  mutate(ola = car::recode(ola, 'c11 == 1 1' = '1', 'c11 == 1 3' = '2', 'c43 == 1 5' = '3', 
                      'c54 == 1 6' = '4', 'c50 == 1 6' = '5', 'c52 == 1 6' = '6',
                      'c56 == 1 6' = '7')) %>%
...
```{r graf-convencion, fig.cap = 'Confianza en convención por plebiscito de salida'}

elsoc_long_2016_2022 %>% 
  filter(tipo_atricion == 1 | tipo_atricion == 33 & ola %in% c(1,3,5,6) &
           !is_nsnr(c05_14, salida)) %>% 
  ...
  ggtitle('Confianza en la Convención Constitucional según voto en plebiscito de salida',
          subtitle = 'Confía mucho o bastante...')
...
elsoc_long_2016_2022 %>%
  ...
  ggtitle('Partidismo, según voto en plebiscito de salida',
          subtitle = 'Porcentaje que es miembro de un partido o movimiento')
...
elsoc_long_2016_2022 %>%
  ...
  theme(plot.caption = element_text(hjust = 0),
        ...
        legend.title = element_blank()) + 
  ggtitle('Partidismo, según voto en plebiscito de salida',
          subtitle = 'Porcentaje no se identifica con ningún partido político')
...
elsoc_long_2016_2022 %>% 
  ...
  select(c14_01, c14_02, c08_04, ola, ponderador02, segmento_disenno, estrato_disenno, salida) %>% 
...
elsoc_long_2016_2022 %>% 
  ...
  select(c05_01, c05_02, c05_07, c05_08, ola, ponderador02, segmento_disenno, estrato_disenno, salida) %>% 
...
elsoc_long_2016_2022 %>%
  filter(tipo_atricion == 1 | tipo_atricion == 33) %>% 
  as_label(ola) %>%
  filter(!is.na(c22), !is.na(salida)) %>%  
  mutate(salida = factor(car::recode(salida, "1 = 1; 2 = 2; 3:5 = 3"), 
                       levels = 1:3,
                       labels = c("Apruebo", "Rechazo", "Otros"))) %>%
  prop(c22 %in% 4:5, by = c(ola, salida), na.rm = T)  %>%
  ggplot(aes(y = prop, x = ola, color = salida, group = salida,
             label = scales::percent(prop, accuracy = .1))) +
  theme_bw() + 
  geom_text_repel(nudge_y = .01, size = 3, color = 'black') +
  ...
  theme(plot.caption = element_text(hjust = 0),
        legend.position = 'top',
        legend.title = element_blank()) + 
  ggtitle('Participación en movimiento social más valorado, según voto en plebiscito de salida',
          subtitle = 'Participa frecuente o muy frecuentemente...')
...
elsoc_long_2016_2022 %>% 
  ...
  select(f05_01, f05_02, ola, ponderador02, segmento_disenno, estrato_disenno, salida) %>% 
...
elsoc_long_2016_2022 %>% 
  ...
  select(f05_03, f05_04, f05_07, ola, ponderador02, segmento_disenno, estrato_disenno, salida) %>% 
