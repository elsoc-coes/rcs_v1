...
```{r graf-tipo-participacion, fig.cap = 'Participación electoral, según tipo de votante'}
...
```{r graf-eficacia, fig.cap = 'Auto-eficacia política, según tipo de votante'}
...
```{r graf-parti-mov-2, fig.cap = 'Participación en marchas o manifestaciones, según tipo de votante'}
...
```{r graf-sexo-2, fig.cap = 'Sexo, según tipo de votante'}
...
```{r graf-edad-2, fig.cap = 'Tramos etarios en 2022, según tipo de votante'}
...
```{r graf-educacion-2, fig.cap = 'Nivel educacional en 2022, según tipo de votante'}
...
```{r graf-clase-subj-ingreso, fig.cap = 'Clase social en 2022, según tipo de votante'}
...
elsoc_long_2016_2022 %>% 
  ...
  theme(plot.caption = element_text(hjust = 0),
        ...
        legend.position = 'top',
        legend.title = element_blank()) + 
  labs(caption = "Nota: Clase social según ingresos fue construida según deciles de ingreso, donde deciles 1-4 corresponden a\nclase baja, deciles 5-9 a clase media y decil 10 a clase alta.")
...
elsoc_long_2016_2022 %>% 
  ...
  labs(caption = "Nota: El gráfico incluye a la muestra encuestada pre y post-plebiscito. La opción no vota solo fue incorporada post-plebiscito como voto retrospectivo.")
...
elsoc_long_2016_2022 %>% 
  ...
  geom_text(data = function(x) { group_by(.data = x, ola, pos_id) %>%
      ...
      'white', 'white', 'black')) +
  guides(fill = guide_legend(reverse = TRUE)) + 
  labs(caption = "Nota: Categoría indeciso o no vota incluye a quienes no votaron, NS/NR, nulos y blancos.")

```

```{r graf-alluvial-pleb-periodo, fig.cap = 'Preferencias en plebiscito de entrada y salida, según periodo del levantamiento'}

elsoc_long_2016_2022 %>% 
  left_join(elsoc_long_2016_2022 %>% 
              filter(ola == 6) %>%
              mutate(post_plebiscito = factor(ifelse(!is.na(c55), 1, 2),
                                              levels = 1:2,
                                              labels = c('Muestra pre-plebiscito de salida', 
                                                         'Muestra post-plebiscito de salida'))) %>%
              select(idencuesta, post_plebiscito), 
            by = 'idencuesta') %>% 
  filter(tipo_atricion == 1, ola %in% 5:6) %>% 
      mutate(salida = factor(car::recode(salida, "1 = 1; 2 = 2; 3:5 = 3")),
         post = is.na(c55),
         c44 = case_when(c44 == 1 ~ 1,
                         c44 == 2 ~ 2,
                         c44 == 3 ~ 3,
                         c44 == 4 ~ 3,
                         c44 == -999 ~ 3,
                         ...
                         c43 == -777 ~ 3,
                         c43 == -666 ~ 3),
         pos_id = ifelse(!is.na(c44), c44, salida),
         pos_id = factor(pos_id,
                         levels = 1:3,
                         labels = c("Apruebo", "Rechazo", "Indeciso o\nno vota")),
             ola = factor(ola,
                          ...
                          labels = c("Plebiscito de\nEntrada\n2020",
                                     "Plebiscito de\nSalida\n2022"))) %>% 
  survey_design_elsoc() %>%
  svytable(~ post_plebiscito + pos_id + ola + idencuesta, design = .) %>% 
  as.data.frame() %>% 
  group_by(ola, post_plebiscito) %>% 
  mutate(prop = Freq/sum(Freq, na.rm = TRUE)) %>%
  ungroup() %>% 
  filter(Freq > 0) %>%
  ggplot(aes(y = prop, x = ola, fill = fct_rev(pos_id), 
  ...
  ggalluvial::geom_stratum(linetype = 0, width = .55) +
  scale_y_continuous(labels = scales::percent) + 
  scale_fill_viridis_d(direction = -1, end = .8, option = 'viridis') +
  ylab(label = NULL) +
  xlab(label = NULL) + 
  theme(plot.caption = element_text(hjust = 0),
        legend.position = 'top',
        legend.title = element_blank()) +
  geom_text(data = function(x) { group_by(.data = x, post_plebiscito, ola, pos_id) %>%
      summarise(prop = sum(prop, na.rm = TRUE), idencuesta = 1) %>%
      ungroup()},
            aes(label = scales::percent(prop, accuracy = .1)),
            position = position_stack(vjust = .5),
...
```{r graf-tipo-salida, fig.cap = 'Preferencias en plebiscito de salida, según tipo de votante'}
...
```{r graf-alluvial-pleb-tipo, fig.cap = 'Preferencias de voto en plebiscito de entrada y salida, según tipo de votante'}
...
```{r graf-consti-2, fig.cap = 'Nueva constitución, según tipo de votante'}
...
```{r graf-convencion-2, fig.cap = 'Desconfianza en la Convención Constitucional en 2022, según tipo de votante'}
...
```{r graf-leg-dem-2, fig.cap = 'Legitimidad democrática, según tipo de votante'}
...
```{r graf-autori, fig.cap = 'Preferencia por el autoritarismo, según tipo de votante'}
...
```{r graf-satis-dem-2, fig.cap = 'Satisfacción democrática, según tipo de votante'}
...
```{r graf-autoritarismo-2, fig.cap = 'Actitudes autoritarias, según tipo de votante'}
...
```{r graf-posicion-pol-2, fig.cap = 'Posición política en 2022, según tipo de votante'}
...
```{r graf-sin-posicion-2, fig.cap = 'Ausencia de posición política, según tipo de votante'}
...
```{r graf-cambios-idpol-idpol, fig.cap = 'Cambios en posición política entre 2019 y 2022, según tipo de votante'}
...
```{r graf-interes-2, fig.cap = 'Interés en la política, según tipo de votante'}
...
```{r graf-id-partido-2, fig.cap = 'Partidismo, según tipo de votante'}
...
```{r graf-involv-poli-2, fig.cap = 'Involucramiento político, según tipo de votante'}
...
```{r graf-conf-pol-2, fig.cap = 'Desonfianza en instituciones políticas, según tipo de votante'}
...
```{r graf-conf-pol-2-2, fig.cap = 'Desconfianza en instituciones políticas, según tipo de votante'}
...
```{r graf-carab, fig.cap = 'Desonfianza en Carabineros, según tipo de votante'}
...
```{r graf-op-valores-2, fig.cap = 'Actitudes valóricas, según tipo de votante'}
...
```{r graf-pension, fig.cap = '"Cada persona debiera asegurarse por si mismo su futura pensión para la tercera edad", según tipo de votante'}
...
```{r graf-educacion, fig.cap = '"El Estado de Chile, más que los privados, debería ser el principal proveedor de educación", según tipo de votante'}
...
```{r desconf-mapuche, fig.cap = 'Desconfianza hacia los Mapuche, según tipo de votante'}
...
```{r consti-mapuche, fig.cap = '"La nueva constitución permitirá una mejor relación entre el Estado y los pueblos indígenas en Chile" (2022), según tipo de votante'}
...
```{r desconf-inmigrante, fig.cap = 'Desconfianza hacia inmigrantes, según nacionalidad de migrantes y tipo de votante'}
...
```{r ame-realista-proced, fig.cap = 'Inmigración y desempleo, según nacionalidad de migrantes y tipo de votante'}
...
```{r ame-simbolica-proced, fig.cap = 'Inmigración e identidad chilena, según nacionalidad de migrantes y tipo de votante'}
