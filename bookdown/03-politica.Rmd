...
elsoc_long_2016_2022 <- elsoc_long_2016_2022 %>%
  left_join(perfiles, by = "idencuesta") %>%
   mutate(pp_3 = factor(pp_3, 
                       levels = 1:3,
                       labels = c("Votante\nHabitual (70.21%)", "No-Votante (17.06%)", "Activado (12.73%)"))) %>%
...
elsoc_wide_2016_2022 <- elsoc_wide_2016_2022 %>%
  left_join(perfiles, by = "idencuesta") %>%
   mutate(pp_3 = factor(pp_3, 
                       levels = 1:3,
                       labels = c("1 (70.21%)", "2 (17.06%)", "3 (12.73%)"))) %>%
   mutate(pp_4 = factor(pp_4, 
                       levels = 1:4,
                       labels = c("1 (10.90%)", "2 (8.29%)", "3 (69.28%)", "4 (11.53%)"))) %>%
   mutate(pp_5 = factor(pp_5, 
...
elsoc_long_2016_2022 %>% 
  ...
  ggtitle('Nueva constitución según ola',
          subtitle = 'Nivel de acuerdo con cambiar la constitución')
...
elsoc_long_2016_2022 %>% 
  filter(tipo_atricion == 1 | tipo_atricion == 33) %>% 
  filter(!is.na(c12_03)) %>%
  mutate(c12_03 = factor(car::recode(c12_03, "1 = 1; 2:3 = 2"), 
                       levels = 1:2,
                       labels = c("No miembro", 
                                  "Miembro activo\n o inactivo"))) %>%
  sjlabelled::as_label(ola) %>% 
  prop(x = c12_03, by = ola, na.rm = TRUE) %>% 
  ggplot(aes(y = prop, x = ola, fill = fct_rev(c12_03), 
             label = as.character(scales::percent(prop, accuracy = .1)))) + 
  theme_bw() + 
  geom_col(position = 'Stack') +
  scale_y_continuous(labels = scales::percent) +
