...
```{r salud-bienestar, fig.cap = 'Satisfacción con la vida, ideal de vida y salud subjetiva (ola 2021)'}
...
miau = elsoc_long_2016_2022 %>% 
  ...
  filter(ola == 6,!is.na(salida)) %>% 
  prop(salida, by = c(sit_laboral), na.rm = TRUE) %>% 
  sjlabelled::as_label(salida, m02) %>% 
  ggplot(aes(y = prop, x = m02, fill = fct_rev(salida), 
             label = scales::percent(prop, accuracy = .1))) + 
  theme_bw() + 
  geom_col(position = 'Stack') +
  scale_y_continuous(labels = scales::percent) +
  ...
  geom_text(position = position_stack(vjust = .5),
            size = 3, 
            color = rep(rep(c('black', 'white'), each = 2), 9)) + 
    theme(plot.caption = element_text(hjust = 0),
          legend.position = 'top',
        legend.title = element_blank()) + 
  guides(fill = guide_legend(reverse = TRUE)) +
