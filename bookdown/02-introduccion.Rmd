...
ELSOC tiene un compromiso con los más altos estándares científicos en términos de producción y análisis de datos. Dentro de esta visión global, ELSOC se guía por las principales pautas de Transparencia y Apertura en la investigación científica. Por esta misma razón, los códigos utilizados para el desarrollo de este documento se encontrarán disponibles en https://github.com/edgardo-cerda/reporte-elsoc-2022.
...
bind_rows(
  ...
  prop_list(edadt, by = c(muestra), na.rm = TRUE)),
  ...
  mutate(edadt = factor(car::recode(m0_edad, "18:29 = 1; 30:49 = 2; 50:64 = 3; 65:150 = 4"),
         ...
         ponderador0 = 1) %>%
  survey_design_elsoc(weights = ponderador0) %>% 
  prop_list(edadt,by=muestra, na.rm = TRUE))) %>%
...
```{r graf-edadt-muestra, fig.cap='Composición de muestra longitudinal: Sexo'}
...
bind_rows(
  ...
  mutate(muestra = ifelse(muestra==1,"Original","Refresco"),
         ponderador0 = 1) %>%
  survey_design_elsoc(weights = ponderador0) %>% 
  prop_list(m0_sexo, by = c(muestra), na.rm = TRUE)),
  ...
  mutate(muestra = "Total",
         ponderador0 = 1) %>%
  survey_design_elsoc(weights = ponderador0) %>% 
  prop_list(m0_sexo,by=muestra, na.rm = TRUE))) %>%
...
```{r graf-educ-muestra, fig.cap='Composición de muestra longitudinal: Educación'}
...
bind_rows(
  ...
  mutate(educ = factor(car::recode(m01, recodes = "1:3 = 1; 4:5 = 2; 6:7 = 3; 8:10 = 4"),
         ...
         ponderador0 = 1) %>%
  survey_design_elsoc(weights = ponderador0) %>% 
  prop_list(educ, by = c(muestra), na.rm = TRUE)),
  ...
  mutate(educ = factor(car::recode(m01, recodes = "1:3 = 1; 4:5 = 2; 6:7 = 3; 8:10 = 4"),
         ...
         ponderador0 = 1) %>%
  survey_design_elsoc(weights = ponderador0) %>% 
  prop_list(educ,by=muestra, na.rm = TRUE))) %>%
...
bind_rows(
  ...
  mutate(estrato = factor(car::recode(estrato, recodes = "5:6=5"),
         ...
         ponderador0 = 1) %>%
  survey_design_elsoc(weights = ponderador0) %>% 
  prop_list(estrato, by = c(muestra), na.rm = TRUE)),
  ...
  mutate(estrato = factor(car::recode(estrato, recodes = "5:6=5"),
         ...
         ponderador0 = 1) %>%
  survey_design_elsoc(weights = ponderador0) %>% 
  prop_list(estrato,by=muestra, na.rm = TRUE))) %>%
